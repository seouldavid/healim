...
<!-- 메인 테이블--------------------------------------------------------------------------- -->
    ...
    <div class="top2">
        ...
        <ul class="menu">
          ...
          <li style="background-color: lightgray;" onclick="location.href='/notice_list';">공지사항</li>
	        ...
	        <table style="text-align: center;">
	          ...
	          <c:choose>
	         	<c:when test="${empty notice_list}">
	          	...
	          	<c:otherwise>
	          		<c:forEach var="k" items="${notice_list}" varStatus="c">
	          		<tr>
	          		<td>공지사항</td>
	          		<td>${k.mber_nm}</td>
	          		<td><a href="/notice_detail?wr_id=${k.wr_id }&cPage=${paging.nowPage}">${k.wr_subject}</a></td>
					<td>${k.crt_dt}</td>
					<td>
						<input type="button" value="수정" onclick="notice_update(${k.wr_id}, ${paging.nowPage})">
						<input type="button" value="삭제" onclick="notice_delete(${k.wr_id}, ${paging.nowPage})">
					</td>
	          		</tr>
	          		</c:forEach>
	          	</c:otherwise>
					...
					<td colspan="4">
						<ol class="paging">
							...
							<c:choose>
								...
								<c:otherwise>
									<li><a href="/notice_list?cPage=${paging.beginBlock - paging.pagePerBlock }">이전</a></li>
								</c:otherwise>
							</c:choose>
							<!-- 페이지번호 -->
							<c:forEach begin="${paging.beginBlock}" end="${paging.endBlock }" step="1" var="k">
								<c:choose>
								    ...
								    <c:otherwise>
								    		<li><a href="/notice_list?cPage=${k}">${k }</a></li>
								    </c:otherwise>
								</c:choose>
							</c:forEach>
							<!-- 다음 버튼 -->
							<c:choose>
								...
								<c:otherwise>
									<li><a href="/notice_list?cPage=${paging.beginBlock + paging.pagePerBlock }">다음</a></li>
								</c:otherwise>
							</c:choose>
						</ol>
					</td>
					<td style="text-align: center;"><input type="button" value="공지 등록" onclick="notice_write(${paging.nowPage})" style="width: 80px;"></td>
				</tr>
			</tfoot>
	        </table>
      </section>
...
<!--푸터 --------------------------------------------------------------------------- -->
    ...
    <script type="text/javascript">
		...
		function notice_update(wr_id, nowPage) {
			location.href = "/notice_update?wr_id=" + wr_id + "&cPage=" + nowPage;
		}
		function notice_delete(wr_id, nowPage) {
			if (confirm('정말 이 게시글을 삭제하시겠습니까?')) {
			location.href = "/notice_delete?wr_id=" + wr_id + "&cPage=" + nowPage;
		    }
		}
	</script>
